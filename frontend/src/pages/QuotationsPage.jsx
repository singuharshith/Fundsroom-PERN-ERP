...
import { Plus, FileText, Eye, X, Check, XCircle, Send, ShoppingBag, Trash2, ArrowRight, Calculator } from 'lucide-react';
    ...
    <div class="space-y-6">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div class="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
          <span class="text-xs font-bold uppercase tracking-wider text-slate-400">Total Quotations</span>
          <div class="text-2xl font-black text-slate-900 mt-1">{totalQuotationsCount}</div>
          <span class="text-xs text-slate-500 font-medium">Drafted or Sent</span>
        </div>

        <div class="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
          <span class="text-xs font-bold uppercase tracking-wider text-emerald-600">Accepted Deals</span>
          <div class="text-2xl font-black text-emerald-600 mt-1">{acceptedCount}</div>
          <span class="text-xs text-slate-500 font-medium">Ready for Sales Order conversion</span>
        </div>

        <div class="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
          <span class="text-xs font-bold uppercase tracking-wider text-blue-600">Total Quoted Value</span>
          ...
          <span class="text-xs text-slate-500 font-medium">Server-side calculated value</span>
        </div>
      </div>
      ...
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
        <div>
          <h1 class="text-xl font-bold text-slate-900 flex items-center gap-2">
            ...
            Pricing Quotations (Step 2)
          </h1>
          <p class="text-xs text-slate-500 mt-0.5">
            Formally quote prices, discount %, and GST %. Backend computes exact grand totals.
          </p>
        </div>
        <button
          onClick={() => { resetForm(); setIsCreateOpen(true); }}
          class="flex items-center gap-2 px-4 py-2.5 bg-purple-600 hover:bg-purple-700 text-white text-xs font-bold rounded-xl shadow-md transition-all hover:shadow-purple-500/20"
        >
          <Plus class="w-4 h-4" />
          Draft New Quotation
        </button>
      ...
      <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        {loading ? (
          <div class="p-12 text-center text-slate-500">Loading pricing quotations...</div>
        ) : quotations.length === 0 ? (
          ...
          <div class="overflow-x-auto">
            <table class="w-full text-left text-sm text-slate-600">
              <thead class="bg-slate-50 border-b border-slate-200 text-[11px] font-bold text-slate-500 uppercase tracking-wider">
                <tr>
                  <th class="px-6 py-3.5">Quotation ID</th>
                  <th class="px-6 py-3.5">Customer Name</th>
                  <th class="px-6 py-3.5">Ref Enquiry #</th>
                  <th class="px-6 py-3.5">Valid Until</th>
                  <th class="px-6 py-3.5">Grand Total (₹)</th>
                  <th class="px-6 py-3.5">Status</th>
                  <th class="px-6 py-3.5 text-right">Pipeline Actions</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-slate-100">
                {quotations.map((quo) => (
                  <tr key={quo.id} class="hover:bg-slate-50/80 transition-colors">
                    <td class="px-6 py-4 font-bold text-purple-700 font-mono text-xs">
                    ...
                    <td class="px-6 py-4">
                      <span class={`inline-flex items-center px-2.5 py-1 text-[11px] font-bold rounded-lg border ${getStatusBadgeClass(quo.status)}`}>
                        {quo.status}
                      </span>
                    </td>
                    <td class="px-6 py-4 text-right space-x-1.5">
                      <button
                        ...
                        class="p-1.5 text-slate-600 hover:bg-slate-100 rounded-lg transition-colors inline-flex items-center gap-1 text-xs font-bold"
                        title="View Details"
                      >
                      ...
                      {quo.status === 'DRAFT' && (
                        <button
                          ...
                          class="px-2.5 py-1 bg-blue-50 text-blue-700 hover:bg-blue-100 rounded-lg text-xs font-bold transition-colors inline-flex items-center gap-1"
                        >
                          <Send class="w-3 h-3" />
                          Mark Sent
                        </button>
                      ...
                      {(quo.status === 'SENT' || quo.status === 'DRAFT') && (
                        <>
                          <button
                            ...
                            class="px-2 py-1 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 rounded-lg text-xs font-bold transition-colors inline-flex items-center gap-1"
                          >
                            <Check class="w-3 h-3" />
                            Accept
                          </button>
                          <button
                            ...
                            class="px-2 py-1 bg-rose-50 text-rose-700 hover:bg-rose-100 rounded-lg text-xs font-bold transition-colors inline-flex items-center gap-1"
                          >
                      ...
                      {quo.status === 'ACCEPTED' && !quo.sales_order && (
                        ...
                        >
                          ...
                          Convert to Order (Step 3)
                        </button>
                      )}

                      {quo.sales_order && (
                        <span class="text-[11px] font-mono text-emerald-700 font-bold bg-emerald-50 px-2 py-1 rounded-md border border-emerald-200">
        ...
        <div class="fixed inset-0 z-50 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4">
          <div class="bg-white rounded-2xl shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto border border-slate-200 p-6 space-y-5">
            <div class="flex items-center justify-between pb-3 border-b border-slate-100">
              <div>
                <span class="text-[11px] font-bold text-purple-600 font-mono uppercase tracking-wider">Quotation Summary</span>
                <h3 class="text-lg font-extrabold text-slate-900">{selectedQuotation.quotation_number}</h3>
              </div>
              <button
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 text-xs bg-slate-50 p-4 rounded-xl border border-slate-200/80">
              <div>
                ...
                <span class="font-bold text-slate-900 text-sm">{selectedQuotation.customer?.company_name}</span>
              </div>
              <div>
                <span class="block text-[10px] text-slate-400 font-bold uppercase">Enquiry Ref</span>
                <span class="font-mono text-xs text-blue-600 font-bold">{selectedQuotation.enquiry?.enquiry_number}</span>
              </div>
              <div>
                <span class="block text-[10px] text-slate-400 font-bold uppercase">Valid Until</span>
                <span class="font-semibold text-slate-800">{new Date(selectedQuotation.valid_until).toLocaleDateString()}</span>
              </div>
              <div>
                <span class="block text-[10px] text-slate-400 font-bold uppercase">Status</span>
                <span class={`inline-block mt-0.5 px-2.5 py-0.5 text-xs font-bold rounded-md border ${getStatusBadgeClass(selectedQuotation.status)}`}>
            ...
            <div>
              <h4 class="text-xs font-bold text-slate-900 mb-2 uppercase tracking-wider">Line Items Pricing Matrix (Server Calculated)</h4>
                  ...
                  <tbody class="divide-y divide-slate-100">
                    {selectedQuotation.items?.map((item) => (
                      <tr key={item.id}>
                        ...
                        <td class="px-4 py-3 text-right font-black">{item.quantity}</td>
                        <td class="px-4 py-3 text-right font-mono">₹{parseFloat(item.unit_price).toFixed(2)}</td>
                        <td class="px-4 py-3 text-right font-mono text-slate-600">{parseFloat(item.discount_percent)}%</td>
                        <td class="px-4 py-3 text-right font-mono text-slate-600">{parseFloat(item.gst_percent)}%</td>
                        <td class="px-4 py-3 text-right font-mono font-black text-slate-900">₹{parseFloat(item.line_amount).toFixed(2)}</td>
                      </tr>
                    ))}
                  </tbody>
                  <tfoot class="bg-slate-50 font-bold border-t border-slate-200">
                    <tr>
                      <td colSpan="5" class="px-4 py-3 text-right text-slate-700 uppercase tracking-wider text-[10px]">Calculated Grand Total:</td>
                      <td class="px-4 py-3 text-right font-mono text-sm text-purple-700 font-black">₹{parseFloat(selectedQuotation.grand_total).toFixed(2)}</td>
                    </tr>
                  </tfoot>
                </table>
              </div>
      ...
      {isCreateOpen && (
        <div class="fixed inset-0 z-50 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4">
          <div class="bg-white rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto border border-slate-200 p-6 space-y-5">
            <div class="flex items-center justify-between pb-3 border-b border-slate-100">
              <div>
                <span class="text-[11px] font-bold text-purple-600 uppercase tracking-wider">Step 2 Entry</span>
                <h3 class="text-lg font-extrabold text-slate-900">Draft New Quotation</h3>
              </div>
              <button
            ...
            <form onSubmit={handleCreateSubmit} class="space-y-4 text-xs">
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label class="block font-bold text-slate-700 mb-1">Select Customer Enquiry *</label>
                  <select
                    required
                    value={enquiryId}
                    onChange={(e) => handleEnquirySelect(e.target.value)}
                    class="w-full p-2.5 border border-slate-300 rounded-xl text-xs focus:ring-2 focus:ring-purple-500 font-medium"
                  >
                ...
                <div>
                  <label class="block font-bold text-slate-700 mb-1">Validity Expiration Date *</label>
                  <input
                    type="date"
                    required
                    value={validUntil}
              ...
              <div>
                <div class="flex items-center justify-between mb-2">
                  <label class="block font-bold text-slate-700">Line Items & Discount/GST Pricing Matrix *</label>
                  <button
                    type="button"
                  ...
                  <table class="w-full text-left text-xs">
                    <thead class="bg-slate-50 text-[10px] font-bold text-slate-500 uppercase border-b border-slate-200">
                      <tr>
                        ...
                        <th class="px-2 py-2 w-20 text-center">Disc %</th>
                        <th class="px-2 py-2 w-20 text-center">GST %</th>
                        ...
                        <tr key={index} class="bg-white">
                          <td class="p-2">
                            <select
                              ...
                              class="w-full p-2 border border-slate-300 rounded-lg text-xs font-medium"
                            >
                          ...
                          <td class="p-2">
                            <input
                              ...
                              class="w-full p-2 border border-slate-300 rounded-lg text-xs text-center font-bold"
                            />
                          </td>
                          <td class="p-2">
                            <input
                              ...
                              class="w-full p-2 border border-slate-300 rounded-lg text-xs text-center font-mono font-semibold"
                            />
                          </td>
                          <td class="p-2">
                            <input
                              ...
                              class="w-full p-2 border border-slate-300 rounded-lg text-xs text-center font-mono"
                            />
                          </td>
                          <td class="p-2">
                            <input
                    ...
                    <tfoot class="bg-slate-50 border-t border-slate-200">
                      <tr>
                        <td colSpan="5" class="p-3 text-right font-bold text-slate-700 uppercase tracking-wider text-[10px]">Calculated Grand Total:</td>
                        <td class="p-3 text-right font-mono font-black text-sm text-purple-700">
              ...
              <div class="pt-3 flex items-center justify-end gap-3 border-t border-slate-100">
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  class="px-5 py-2 bg-purple-600 hover:bg-purple-700 text-white text-xs font-bold rounded-xl shadow-md transition-all disabled:opacity-50"
                >
                  {submitting ? 'Saving...' : 'Save & Calculate Quotation'}
                </button>
              </div>
            </form>
          </div>
